...
//            if (scoreBoard.getTotalBall()%6==0) {
//                Alert alert2 = new Alert(Alert.AlertType.ERROR);
//                alert2.setTitle("ERROR");
//                alert2.setHeaderText("You didn't update the run info.");
//                alert2.setContentText("Please update the run info before pressing No Ball.");
//
//            }
...
// Bowlers
    ...
    public void scoreBoardUpdate() {
        ...
        if (scoreBoard.getTotalBall()%6==0 && scoreBoard.getTotalBall()!=0 && bowlerChanged == false && Double.parseDouble(scoreBoard.getTotalOver())!=scoreBoard.getOverLimit()) {
        ...
        Team1.setText(scoreBoard.getTeams()[0].getName());
        Team2.setText(scoreBoard.getTeams()[1].getName());

        lbTotal.setText("Total : " + scoreBoard.getTotalRun() + "/" + scoreBoard.getFallenWickets());
        lbTotalOver.setText("Over : " + scoreBoard.getTotalOver());
        lbExtra.setText("Extra : " + String.valueOf(scoreBoard.getExtraRun()));
        ...
        if (Double.parseDouble(scoreBoard.getTotalOver())==scoreBoard.getOverLimit() && bowlerChanged == false) {
    ...
    public void matchEnd() {
        ...
        ButtonType button = new ButtonType("2nd Innings");
        Alert alert = new Alert(Alert.AlertType.WARNING);
        alert.setTitle("ATTENTION!!!!!!");
        alert.setHeaderText("1st Innings Completed");
        alert.setContentText("Interface will change to Second Innings");
        ...
        String setFileName = team1Name + "Vs" + team2Name + "_" + date;
        String setDatabaseName = setFileName;

        String sql1 = "INSERT INTO BATSMAN1 (batsman_name, out_type, batsman_run, ball_played, four_run, six_run, strike_rate) " +
                "values(?,?,?,?,?,?,?), " +
                "(?,?,?,?,?,?,?), " +
                "(?,?,?,?,?,?,?), " +
                "(?,?,?,?,?,?,?), " +
        ...
        String sql2 = "INSERT INTO BOWLER1 (bowler_name, bowler_over, maiden_over, run_given, wicket_taken, wide_ball, no_ball, economy) " +
                "values(?,?,?,?,?,?,?,?), " +
                "(?,?,?,?,?,?,?,?), " +
                "(?,?,?,?,?,?,?,?), " +
                "(?,?,?,?,?,?,?,?), " +
        ...
        String sql3 = "INSERT INTO matchinfo1 (match_info1) values(?);";
        ...
        try {
            ...
            //saving the match name/database name in the file
            DatabaseNames saveDatabaseName = new DatabaseNames(setFileName);


            //creating the database with the given teamnames
            Connection newDatabase = new DatabaseConnection("root", "").createDatabase(setDatabaseName);
            newDatabase.close();
            ...
            //inserting overall 1st innings info in the batsman1 table in "match_info" column in the database
        ...
        while (true) {
            ...
            try {
                ...
                if (alert.showAndWait().get().getText().equals("2nd Innings")) {


                    FXMLLoader loader = new FXMLLoader(getClass().getResource("SetBatsman2.fxml"));
                    ...
                    SetBatsman2Control setBatsman2Control = loader.getController();
                    setBatsman2Control.setTeam1(scoreBoard.getTeams()[0]);
                    setBatsman2Control.setTeam2(scoreBoard.getTeams()[1]);
                    setBatsman2Control.setOverLimit(scoreBoard.getOverLimit());
                    setBatsman2Control.setComboBox();

                    setBatsman2Control.setScoreBoard00(scoreBoard);
